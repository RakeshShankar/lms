...
import com.maiora.lms.api.service.AssignmentService;
...
public class AssignmentsController {
	 ...
	 * @return
	 */
	@RequestMapping(value = "/assignmnet/from/{from}/pagesize/{pagesize}/filtertext/{filtertext}", method = RequestMethod.GET)
	@ResponseBody
	...
	public ResponseEntity<?> getAllFilters(@PathVariable("from") int from, @PathVariable("pagesize") int pagesize,
		...
		try {
			...
			List<Assignments> assignModel = assignmentServ.readAllAssignments(from, pagesize, filtertext);
			List<AssignmentsDTO> assignDTO = modelMapper.map(assignModel,listType);
	 ...
	 * @return
	 */
	@RequestMapping(value = "/assignment/v1/create", method = RequestMethod.POST)
	@ResponseBody
	@ApiOperation(value = "Creating Address", notes = "API Used To Create A New Address")
	public ResponseEntity<?> createNewAdds(@Validated @RequestBody AssignmentsDTO assignmentDTO) {
		try {
			ModelMapper modelMapper = new ModelMapper();
			Assignments assignmentModel = modelMapper.map(assignmentDTO, Assignments.class);
			Assignments result = assignmentServ.addAssignments(assignmentModel);
			return new ResponseEntity<Assignments>(result, HttpStatus.OK);
		} catch (Exception ex) {
			String errorMessage;
			errorMessage = ex.getMessage();
			return new ResponseEntity<>(errorMessage, HttpStatus.BAD_REQUEST);
	 ...
	 * @return
	 */
	@RequestMapping(value = "/assignment/v1/update", method = RequestMethod.PUT)
	@ResponseBody
	@ApiOperation(value = "Updateing The address Based On The Necessaity", notes = "Updating The address")
	public ResponseEntity<?> updateOutlet(@RequestBody AssignmentsDTO assignDTO) {
		try {
			ModelMapper modelMapper = new ModelMapper();
			Assignments assignmentModel = modelMapper.map(assignDTO,Assignments.class);
			Assignments result = assignmentServ.updateTheExistingAssignment(assignmentModel);
			return new ResponseEntity<Assignments>(result, HttpStatus.OK);
		} catch (Exception ex) {
			String errorMessage = "";
			if (ex.getCause() != null) {
	 ...
	 * @return
	 */
	@RequestMapping(value = "/assignment/v1/getAll", method = RequestMethod.GET)
	@ResponseBody
