...
import com.maiora.lms.api.dto.UsersDTO;
import com.maiora.lms.api.model.Assignments;
import com.maiora.lms.api.model.Users;
import com.maiora.lms.api.service.UsersService;
...
public class UsersController {
	 ...
	 * @return
	 */
	@RequestMapping(value = "/user/from/{from}/pagesize/{pagesize}/filtertext/{filtertext}", method = RequestMethod.GET)
	@ResponseBody
	...
	public ResponseEntity<?> getAllFilters(@PathVariable("from") int from, @PathVariable("pagesize") int pagesize,
		...
		try {
			...
			List<Users> usersModel = usersService.readAllUsers(from, pagesize, filtertext);
			List<UsersDTO> usersDTO = modelMapper.map(usersModel,listType);
	...
	public ResponseEntity<?> createNewAdds(@Validated @RequestBody UsersDTO usersDTO) {
		try {
			ModelMapper modelMapper = new ModelMapper();
			Users usersModel = modelMapper.map(usersDTO, Users.class);
			Users result = usersService.addUsers(usersModel);
	 ...
	 * @return
	 */
	@RequestMapping(value = "/user/v1/update", method = RequestMethod.PUT)
	@ResponseBody
	@ApiOperation(value = "Updateing The address Based On The Necessaity", notes = "Updating The address")
	public ResponseEntity<?> updateOutlet(@RequestBody UsersDTO usersDTO) {
		try {
			ModelMapper modelMapper = new ModelMapper();
			Users usersModel = modelMapper.map(usersDTO,Users.class);
			Users result = usersService.updateTheExistingUsers(usersModel);
			return new ResponseEntity<Users>(result, HttpStatus.OK);
		} catch (Exception ex) {
			String errorMessage = "";
			if (ex.getCause() != null) {
	 ...
	 * @return
	 */
	@RequestMapping(value = "/user/v1/getAll", method = RequestMethod.GET)
	@ResponseBody
	...
	public ResponseEntity<?> getAllFilters() {
		...
		try {
			...
			List<Users> sellerModel = usersService.getAllData();
			List<UsersDTO> sellerDTO = modelMapper.map(sellerModel,listType);
